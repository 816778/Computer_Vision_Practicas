...
def process_matches(params, K, draw_graphic=False):
    ...
    Xw, x1, x2, T_wc1, T_wc2, triangulated_data, state = triangulation.triangulate_from_superglue_pair(data, K, params, state, idx_file=0)
    ...
    T_opt, X_w_opt = bundle.run_bundle_adjustment([T_wc1, T_wc2], K, Xw, [x1, x2], False)
    ...
    for idx, path in enumerate(path_new_images[2:], start=3):
        ...
        pts3, pts2, state, T_wc, state = triangulation.calculate_new_pose(params, name, state, K, name=name, verbose=True)     
        print(f"[INFO] Points 3D: {pts3.shape}, Points 2D: {pts2.shape}")
        
        cameras_opt[f'C{idx}'] = T_wc
        plot_utils.plot3DPoints(pts3.T, cameras_opt, world_ref=False)
